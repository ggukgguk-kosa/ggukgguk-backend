...
public class ReplyServiceImpl implements ReplyService {
	...
	public List<ReplyNickname> addReply(Reply reply) {
		...
		try {
			...
			List<ReplyNickname> record = dao.selectReplies(reply.getRecordId());
			
//			// 방금 생성한 댓글 테이블의 아이디 값을 가져오기
//			int replyId = (int)reply.getReplyId();	
			
			// 이후 새로운 댓글 알림 생성 
			Notification noti = new Notification(0, "NEW_REPLY", new Date(), record.get(0).getRecordId(), record.get(0).getMemberId(),0, reply.getMemberId() + "님이 댓글을 남겼습니다.");
